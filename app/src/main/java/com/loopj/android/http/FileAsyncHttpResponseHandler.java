...
public abstract class FileAsyncHttpResponseHandler extends AsyncHttpResponseHandler {
    ...
    public byte[] getResponseData(HttpEntity httpEntity) throws UnsupportedOperationException, IOException {
        ...
        try {
            ...
            int i10 = 0;
            while (true) {
                int i11 = content.read(bArr);
                if (i11 == -1 || Thread.currentThread().isInterrupted()) {
                    break;
                }
                i10 += i11;
    ...
    public File getTargetFileByParsingURL() {
        ...
        if (strSubstring.contains(".")) {
            strConcat = strSubstring.substring(0, strSubstring.lastIndexOf(46)) + " (%d)" + strSubstring.substring(strSubstring.lastIndexOf(46), strSubstring.length());
        } else {
            strConcat = strSubstring.concat(" (%d)");
        }
        int i10 = 0;
        while (true) {
            File file2 = new File(getOriginalFile(), String.format(strConcat, Integer.valueOf(i10)));
            if (!file2.exists()) {
                return file2;
            }
            i10++;
    ...
    public abstract void onFailure(int i10, Header[] headerArr, Throwable th2, File file);

    @Override // com.loopj.android.http.AsyncHttpResponseHandler
    public final void onFailure(int i10, Header[] headerArr, byte[] bArr, Throwable th2) {
        onFailure(i10, headerArr, th2, getTargetFile());
    }
    ...
    public FileAsyncHttpResponseHandler(File file, boolean z10, boolean z11, boolean z12) {
        super(z12);
