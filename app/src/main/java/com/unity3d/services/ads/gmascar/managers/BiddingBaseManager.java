...
public abstract class BiddingBaseManager implements IBiddingManager {
    ...
    public final void onUnityAdsTokenReady(String str) throws IllegalAccessException, NoSuchMethodException, SecurityException, IllegalArgumentException, InvocationTargetException {
        if (this.unityAdsTokenListener != null) {
            Utilities.wrapCustomerListener(new x(18, this, str));
        }
        if (str == null || str.isEmpty()) {
            return;
        }
            ...
            new Thread(new Runnable() { // from class: com.unity3d.services.ads.gmascar.managers.BiddingBaseManager.2
                ...
                public void run() {
                    try {
                        ...
                        BiddingBaseManager.this.getMetricSender().sendMetric(ScarMetric.hbSignalsUploadFailure(BiddingBaseManager.this._isAsyncTokenCall, e10.getLocalizedMessage()));
                    }
                }
            }).start();
        }
