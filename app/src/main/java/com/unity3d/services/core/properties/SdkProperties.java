...
public class SdkProperties {
    ...
    public static String getDefaultConfigUrl(String str) {
        return "https://" + getConfigVersion(ClientProperties.getApplicationContext()) + ClassUtils.PACKAGE_SEPARATOR_CHAR + (isChinaLocale(Device.getNetworkCountryISO()) ? new String(Base64.decode(CHINA_CONFIG_HOSTNAME, 0)) : DEFAULT_CONFIG_HOSTNAME) + "/webview/" + getWebViewBranch() + "/" + str + "/config.json";
    }
    ...
    public static void notifyInitializationComplete() throws IllegalAccessException, NoSuchMethodException, SecurityException, IllegalArgumentException, InvocationTargetException {
        ...
        for (IUnityAdsInitializationListener iUnityAdsInitializationListener : getInitializationListeners()) {
            ...
            Utilities.wrapCustomerListener(new d(iUnityAdsInitializationListener, 24));
        }
        resetInitializationListeners();
    }

    public static void notifyInitializationFailed(UnityAds.UnityAdsInitializationError unityAdsInitializationError, String str) throws IllegalAccessException, NoSuchMethodException, SecurityException, IllegalArgumentException, InvocationTargetException {
        setInitializeState(InitializationState.INITIALIZED_FAILED);
        for (IUnityAdsInitializationListener iUnityAdsInitializationListener : getInitializationListeners()) {
            Utilities.wrapCustomerListener(new g(5, iUnityAdsInitializationListener, unityAdsInitializationError, str));
        }
        resetInitializationListeners();
    }
