...
public class ObjectReader extends ObjectCodec implements Serializable {
    ...
    public <T> MappingIterator<T> _detectBindAndReadValues(DataFormatReaders.Match match, boolean z10) throws IOException {
        if (!match.hasMatch()) {
    ...
    public Object _detectBindAndClose(DataFormatReaders.Match match, boolean z10) throws IOException {
        if (!match.hasMatch()) {
            _reportUnkownFormat(this._dataFormatReaders, match);
        }
        JsonParser jsonParserCreateParserWithMatch = match.createParserWithMatch();
    ...
    public JsonNode readTree(byte[] bArr, int i10, int i11) throws IOException {
        _assertNotNull("json", bArr);
        if (this._dataFormatReaders != null) {
            _reportUndetectableSource(bArr);
        }
        return _bindAndCloseAsTree(_considerFilter(this._parserFactory.createParser(bArr, i10, i11), false));
    }
    ...
    public <T> T readValue(byte[] bArr, int i10, int i11) throws IOException {
        ...
        return (T) _bindAndClose(_considerFilter(this._parserFactory.createParser(bArr, i10, i11), false));
    }
