...
public final class RetryAndFollowUpInterceptor implements Interceptor {
    ...
    private final Request buildRedirectRequest(Response response, String str) {
        ...
        if (!i.areEqual(httpUrlResolve.scheme(), response.request().url().scheme()) && !this.client.followSslRedirects()) {
            return null;
        }
    ...
    private final int retryAfter(Response response, int i10) throws NumberFormatException {
        String strHeader$default = Response.header$default(response, cz.msebera.android.httpclient.HttpHeaders.RETRY_AFTER, null, 2, null);
        ...
        i.checkNotNullExpressionValue(numValueOf, "valueOf(header)");
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        List listEmptyList = k.emptyList();
            ...
            try {
                ...
                try {
                    ...
                    if (!recover(e10, call$okhttp, request$okhttp, !(e10 instanceof ConnectionShutdownException))) {
                        throw Util.withSuppressed(e10, listEmptyList);
                    }
                    listEmptyList = q.plus(listEmptyList, e10);
                    call$okhttp.exitNetworkInterceptorExchange$okhttp(true);
                    z10 = false;
                } catch (RouteException e11) {
                    if (!recover(e11.getLastConnectException(), call$okhttp, request$okhttp, false)) {
                        throw Util.withSuppressed(e11.getFirstConnectException(), listEmptyList);
                    }
                    listEmptyList = q.plus(listEmptyList, e11.getFirstConnectException());
                    call$okhttp.exitNetworkInterceptorExchange$okhttp(true);
                    z10 = false;
                }
                ...
                i10++;
                if (i10 > 20) {
                    throw new ProtocolException(i.stringPlus("Too many follow-up requests: ", Integer.valueOf(i10)));
                }
                call$okhttp.exitNetworkInterceptorExchange$okhttp(true);
