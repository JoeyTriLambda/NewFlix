...
public class TypeUtils {
    ...
    private static int[] findRecursiveTypes(ParameterizedType parameterizedType) {
        ...
        for (int i10 = 0; i10 < typeArr.length; i10++) {
            ...
            if ((type instanceof TypeVariable) && containsVariableTypeSameParametrizedTypeBound((TypeVariable) type, parameterizedType)) {
                iArrAdd = ArrayUtils.add(iArrAdd, i10);
            }
        }
        return iArrAdd;
    }
    ...
    private static String parameterizedTypeToString(ParameterizedType parameterizedType) {
        ...
        if (iArrFindRecursiveTypes.length > 0) {
            appendRecursiveTypes(sb2, iArrFindRecursiveTypes, parameterizedType.getActualTypeArguments());
        } else {
            sb2.append('<');
    ...
    public static String toLongString(TypeVariable<?> typeVariable) {
        ...
        if (genericDeclaration instanceof Class) {
            ...
            while (enclosingClass.getEnclosingClass() != null) {
                sb2.insert(0, enclosingClass.getSimpleName()).insert(0, ClassUtils.PACKAGE_SEPARATOR_CHAR);
                enclosingClass = enclosingClass.getEnclosingClass();
            }
            sb2.insert(0, enclosingClass.getName());
    ...
    private static Type[] unrollBounds(Map<TypeVariable<?>, Type> map, Type[] typeArr) {
        ...
        while (i10 < typeArr.length) {
            Type typeUnrollVariables = unrollVariables(map, typeArr[i10]);
            if (typeUnrollVariables == null) {
                typeArr = (Type[]) ArrayUtils.remove((Object[]) typeArr, i10);
                i10--;
            } else {
                typeArr[i10] = typeUnrollVariables;
            }
            i10++;
        }
        return typeArr;
    ...
    public static Type unrollVariables(Map<TypeVariable<?>, Type> map, Type type) {
        ...
        if (containsTypeVariables(type)) {
            ...
            if (type instanceof ParameterizedType) {
                ...
                Type[] actualTypeArguments = parameterizedType.getActualTypeArguments();
                for (int i10 = 0; i10 < actualTypeArguments.length; i10++) {
                    Type typeUnrollVariables = unrollVariables(map, actualTypeArguments[i10]);
                    if (typeUnrollVariables != null) {
                        actualTypeArguments[i10] = typeUnrollVariables;
                    }
                }
    ...
    private static <T> String toString(T t10) {
        return t10 instanceof Type ? toString((Type) t10) : t10.toString();
    }
