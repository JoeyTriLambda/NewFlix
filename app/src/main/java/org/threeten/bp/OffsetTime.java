...
public final class OffsetTime extends DefaultInterfaceTemporalAccessor implements Temporal, TemporalAdjuster, Comparable<OffsetTime>, Serializable {
    ...
    public int compareTo(OffsetTime offsetTime) {
        ...
        int iCompareLongs = Jdk8Methods.compareLongs(toEpochNano(), offsetTime.toEpochNano());
        return iCompareLongs == 0 ? this.time.compareTo(offsetTime.time) : iCompareLongs;
    }
    ...
    public OffsetTime plus(long j10, TemporalUnit temporalUnit) {
        if (temporalUnit instanceof ChronoUnit) {
            return with(this.time.plus(j10, temporalUnit), this.offset);
        }
        return (OffsetTime) temporalUnit.addTo(this, j10);
    }
    ...
    public OffsetTime with(TemporalField temporalField, long j10) {
        if (temporalField instanceof ChronoField) {
            if (temporalField == ChronoField.OFFSET_SECONDS) {
                return with(this.time, ZoneOffset.ofTotalSeconds(((ChronoField) temporalField).checkValidIntValue(j10)));
            }
            return with(this.time.with(temporalField, j10), this.offset);
        }
        return (OffsetTime) temporalField.adjustInto(this, j10);
    }
}
