            ...
            for (Method method : methods) {
                ...
                if ((modifiers & 1) != 0 && (modifiers & MODIFIERS_IGNORE) == 0) {
                    ...
                    if (parameterTypes.length == 1) {
                        ...
                        StringBuilder sbV = c.v("@Subscribe method ", method.getDeclaringClass().getName() + "." + method.getName(), "must have exactly 1 parameter but has ");
                        sbV.append(parameterTypes.length);
                        throw new EventBusException(sbV.toString());
                    }
                } else if (this.strictMethodVerification && method.isAnnotationPresent(Subscribe.class)) {
                    throw new EventBusException(c.B(method.getDeclaringClass().getName() + "." + method.getName(), " is a illegal @Subscribe method: must be public, non-static, and non-abstract"));
                }
            }
        } catch (LinkageError e10) {
            String strConcat = "Could not inspect methods of ".concat(findState.clazz.getName());
            throw new EventBusException(this.ignoreGeneratedIndex ? c.B(strConcat, ". Please consider using EventBus annotation processor to avoid reflection.") : c.B(strConcat, ". Please make this class visible to EventBus annotation processor to avoid reflection."), e10);
        }
    }
