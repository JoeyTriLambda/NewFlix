...
final class RequestFactory {
    ...
    public static final class Builder {
        ...
        private ParameterHandler<?> parseParameterAnnotation(int i10, Type type, Annotation[] annotationArr, Annotation annotation) {
            ...
            if (annotation instanceof Part) {
                ...
                Headers headersOf = Headers.of("Content-Disposition", ac.c.k("form-data; name=\"", strValue5, "\""), MIME.CONTENT_TRANSFER_ENC, part.encoding());
                if (!Iterable.class.isAssignableFrom(rawType8)) {
                    if (!rawType8.isArray()) {
                        if (MultipartBody.Part.class.isAssignableFrom(rawType8)) {
                        ...
                        return new ParameterHandler.Part(this.method, i10, headersOf, this.retrofit.requestBodyConverter(type, annotationArr, this.methodAnnotations));
                    }
                    ...
                    return new ParameterHandler.Part(this.method, i10, headersOf, this.retrofit.requestBodyConverter(clsBoxIfPrimitive, annotationArr, this.methodAnnotations)).array();
                }
                if (type instanceof ParameterizedType) {
                    ...
                    return new ParameterHandler.Part(this.method, i10, headersOf, this.retrofit.requestBodyConverter(parameterUpperBound4, annotationArr, this.methodAnnotations)).iterable();
                }
                throw Utils.parameterError(this.method, i10, rawType8.getSimpleName() + " must include generic type (e.g., " + rawType8.getSimpleName() + "<String>)", new Object[0]);
            }
    ...
    public Request create(Object[] objArr) throws IOException {
        ...
        if (length != parameterHandlerArr.length) {
            throw new IllegalArgumentException(ac.c.m(ac.c.s("Argument count (", length, ") doesn't match expected count ("), parameterHandlerArr.length, ")"));
        }
